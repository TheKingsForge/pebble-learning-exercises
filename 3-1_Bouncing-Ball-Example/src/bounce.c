// Project 3.1 from Chapter 3 of "Programming Pebble in C"
//
// Bouncy ball program.
//
...
// 5. Changed from constant to static.
static int ball_radius = 25; // 1. 25 is the highest before it doesn't bounce. My theory is that the radius makes the place too high so the hitbox is beyond the edge of the screen causing it to oscillate in place instead of bouncing.

static void select_click_handler(ClickRecognizerRef recognizer, void *context) {
    position_x = 72;
    position_y = 25;
    x_velocity = y_velocity = 2; // 2. Changed velocity
} 

static void up_click_handler(ClickRecognizerRef recognizer, void *context) {
  // 4. Compiler error while just using ball_radius because ball_radius is constant
  ball_radius += 1;
  // 7. Increase speed 
...
static void bounce_the_ball() {
    ...
    // 3. Added X motion. It bounces around like the dvd screensaver. 
